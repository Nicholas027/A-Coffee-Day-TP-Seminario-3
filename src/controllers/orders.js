import { PEER_ERROR, VALIDATION_ERROR } from "../error_management/error.js";
import {
...
export default class Orders {
  ...
  async addOrder(orderData) {
    try {
      ...
      const productsWithAmountToSubtract = orderData.products.map(
        (product) => ({
          ...product,
  ...
  async updateOrder(orderData) {
    try {
      if (orderData.products) {
        ...
        const productsWithAmountDifference = oldOrderedProducts.map(
          (oldOrderedProduct, i) => {
            const difference =
              oldOrderedProduct.amount - orderData.products[i].amount;
